...
## v376.100 — 2026-09-22 🐛 職員のCM番号変更でログインIDを同期（全3split @411×2 / @169 / @266）

事業所職員の介護支援専門員番号を変えてもログインIDが追従せず、個人会員・事業所会員と挙動が違っていた（v376.99 の調査で判明）。

- 承認時に `generateCmBasedLoginId_` で採番し直す。退会者の行が保持するログインIDとの重複を避ける（v376.73 の事故の再発防止）。
- 変わったログインIDを `LOGIN_ID_CHANGED` の設定文面で**職員本人と申請者（代表者）の両方**へ通知する。
- **既存の不一致は触らない**。現に使われているログインIDを奪わないため（operator 判断）。
- `test:staff-login-id-sync` 6 件を追加し prerelease へ連鎖。gas-src から実ソースを切り出して評価する。
- **検証**: prerelease PASS、6/6 PASS、admin 生成物にのみ同期関数が含まれることを確認。実データ確認は未実施。
- 詳細は `docs/286_RELEASE_STATE_v376.100_2026-09-22.md`。

---

## v376.99 — 2026-09-22 🔧 ログインID・メールアドレス変更通知を設定化（全3split @410×2 / @168 / @265）

ログインIDが変わることを公開ポータルの画面で案内していたが、会員マイページを会員へ展開していない現状では受け手に行き場がなく、矛盾した案内になっていた。画面の注意書きをやめ、変わった事実を変わった後にメールで伝える方式へ改めた。

- **新設 2 種別**: `ログインID変更のお知らせ`（認証・セキュリティ）／`メールアドレス変更のお知らせ`（会員手続き）。どちらも管理画面で件名・本文・ON/OFF を編集できる。
- 🐛 **ハードコーディング解消**: 職員情報変更の承認処理に直書きされていたメールアドレス変更通知を、設定の文面へ置き換えた。
- 🐛 **空回りのカード 2 枚を配線**: 会員情報変更確認・退会申請受付は設定キーがあるのに送信箇所が無く、文面を編集しても届かなかった。退会は予定日が承認時にしか確定しないため承認時に送る。既定文面の「（年度末）」直書きも `{{退会方式}}` へ改めた（v376.92 の即時退会で誤表示）。
- 公開ポータルから「⚠ 変更するとログインIDも変わります」を削除。
- **調査**: 事業所職員の介護支援専門員番号を変えてもログインIDは変わらない（ログインは `ログインID` 列で引き、職員とは `職員ID` 列で結ぶためログイン不能にはならない）。個人・事業所と挙動が異なる点は operator 判断待ち。
- **検証**: prerelease PASS、`test:mail-merge-tags` 9/9、GAS 構文チェック PASS、3 split 生成物検査、管理レスポンシブ 7 viewport × 8 view PASS、本番の設定画面で新カードを実画面確認。
- 詳細は `docs/285_RELEASE_STATE_v376.99_2026-09-22.md`。

---

## v376.98.1 — 2026-09-22 🐛 差し込み挿入でテキストボックスが最下部へ飛ばないようにする（全3split @409×2 / @167 / @264）

差し込みボタンを押すと本文が一番下までスクロールし、編集していた場所を見失っていた。未フォーカスの欄へ末尾追記したあと `focus()` がカーソル位置までスクロールしていたのが原因。位置を決めてから `focus({ preventScroll: true })` を呼び、押す前の `scrollTop` を復元する。直近のカーソル位置を欄ごとに覚え、末尾追記になる場面自体も減らした。詳細は `docs/284` §6。

---

## v376.98 — 2026-09-22 🔧 差し込みタグをボタンで挿入（全3split @408×2 / @166 / @263）

差し込み（マージタグ）が一覧表示で、利用者が読んで手で打ち込む必要があった。説明の大半はタグ名の繰り返しだった。

- **クリックで挿入**: カード内のボタンで件名・本文のカーソル位置へ挿入（選択中なら置換）。挿入後はカーソルがタグ直後に戻る。押す前から挿入先（件名／本文）が分かる。
- **説明の削減**: タグ名が意味を含むものは説明を出さない（`氏名`／`会員マイページURL`）。意味を足すものだけ残す（`パスワード` → 初期パスワード）。
- 🐛 **表示タグの不一致を解消**: 事業所・メンバー向けタグが未表示だった件と、職員追加 2 枚・ワークフロー 3 枚が和集合を出して使えないタグ（受付確認の `{{処理備考}}` 等）まで並べていた件。タグはカードのメール種別から引くようにした。
- 🐛 挿入しても送信時に `stripUnresolvedMergeTags_` が黙って落とすため、利用者に理由が分からない状態だった。
- **検証**: prerelease PASS、`test:mail-merge-tags` 9/9、本番で実操作して「カーソル位置へ挿入」を確認、管理レスポンシブ 7 viewport × 8 view PASS。
- 詳細は `docs/284_RELEASE_STATE_v376.98_2026-09-22.md`。

---

## v376.97 — 2026-09-22 🔧 メール通知を系統選択式にする（全3split @407×2 / @165 / @262）

「メール通知の内容」が 14 枚のカードを一度に並べていたため画面が長く、目的の設定に届くまでスクロールが必要だった。

- **系統選択**: 先頭に 6 系統（入会申し込み／職員追加／申請ワークフロー／研修／会員手続き／認証・セキュリティ）のボタンを置き、選んだ系統のカードだけを表示する。一度に出るカードは最大 3 枚になった。
- **状態の可視化**: ボタンに「n 件中 m 件が有効」「n 件すべて停止中」を出し、開かなくても止まっている系統が分かる。
- **「メール配信」は独立したまま**（停止／通常送信／テスト集約）。配信状態は系統に依存しないため。
- 系統を切り替えても入力値は保持し、保存は従来どおり一括保存でまとめて行う。
...
- 詳細は `docs/283_RELEASE_STATE_v376.97_2026-09-22.md`。

---

## v376.96 — 2026-09-18 🔧 メール通知設定のカテゴリ配色（全3split @406×2 / @164 / @261）

「システム設定 ＞ メール通知」の設定カード 14 枚がすべて同じ紫で、文字を読まないとどの設定か判別できなかった。色というチャネルに「有効/無効」しか載っていなかったことが原因。

- **配色**: 色相＝カテゴリ（入会 emerald / 職員 sky / 申請ワークフロー amber / 研修 indigo / 認証・セキュリティ rose / 会員手続き violet）、色の有無＝有効/無効、アイコンの形＝カテゴリの補強。色だけに依存しない（WCAG 1.4.1）。
...
## v376.95.1 — 2026-09-16 🔧 通知設定の整理・Google Chat スレッド化（全3split @405×2 / @163 / @260）

管理画面の通知設定を、メール通知と Google Chat に分離した。メールの全体制御は停止／通常送信／テスト集約の一つに統合し、Google Chat は通知種別・文面のみを管理する。

- **Chat**: 同じ申請IDの受付と処理完了を同一スレッドへ投稿。接続先は秘密設定のままとし、入力欄・表示欄を置かない。
- **互換性**: 旧メール設定から安全に読み替え、既存の通常送信・テスト集約・停止を維持する。初回追加時の矛盾で既存メールを停止させない回帰テストを追加。
- **検証**: prerelease PASS、管理画面の7 viewportレスポンシブ PASS、設定画面の読み取り確認 PASS、fixed deployment 4本同期確認。
- 詳細は `docs/281_RELEASE_STATE_v376.95.1_2026-09-16.md`。

---

## v376.94 — 2026-09-15 🐛 賛助会員の情報変更承認でCM番号検証を誤適用しない（全3split @403×2 / @161 / @258）

賛助会員の連絡先変更を承認したとき、自動採番ログインIDを介護支援専門員番号の代替値として扱い、8桁検証に失敗する不具合を修正した。

- **修正**: 賛助会員ではログインID補完とCM番号の必須・書式検証を行わない。既存の任意値だけは変更せず保全する。
- **既存挙動の維持**: 個人会員のCM番号必須・書式検証、事業所会員の運用、公開本人確認の種別別照合項目は不変。
- **検証**: prerelease PASS、`test:public-identity` 15/15 PASS、3 split 生成物検査 PASS、fixed deployment 4本同期確認。
- 詳細は `docs/280_RELEASE_STATE_v376.94_2026-09-15.md`。

---

## v376.93 — 2026-09-14 🐛 正規E2Eテストデータの申請履歴を限定削除（全3split @402×2 / @160 / @257）

会員削除のcascade対象外だった、会員ID未紐付けの承認済み入会申請がライブ表に残る問題を修正した。admin splitのoperator関数は `test-member-*.invalid` の完全一致だけを対象に、会員・認証・職員・変更申請をsoft deleteする。旧デモ、Dry-run、外部申込者、通常データには触れない。

- **安全性**: 実行前に件数だけを返すプレビューを必須手順とし、物理削除はしない。
- **検証・実行**: prerelease PASS、専用機械検査 PASS、3 split生成物の境界検査 PASS、fixed deployment 4本同期確認。専用プレビューで会員1件・承認済み申請3件だけを確認後に実行し、再プレビューで残件0を確認。
- 詳細は `docs/279_RELEASE_STATE_v376.93_2026-09-14.md`。

---

## v376.92 — 2026-09-12 🆕 退会申込の方式選択・Google Chat 会員手続き通知（全3split @401×2 / @159 / @256）

公開ポータルの退会申込で「年度末退会」と「即時退会」を選択できるようにし、確認画面の表示項目・文言を管理画面で設定可能にした。あわせて、入退会・会員情報変更の申請受付時と確定時を Google Chat へ通知する共通機構を追加した。

- **設定**: 通知の全体有効化、受付・確定・異常値通知、各メッセージ本文は管理画面で編集する。送信先は各 Apps Script project の Script Property に分離し、値をソース・DB・ログ・文書へ保存しない。
- **安全性**: 通知送信の失敗は業務データの書込みや承認を失敗させない。異常値を検出した場合だけ任意通知を追加する。
- **検証**: prerelease PASS、専用の通知・退会方式機械検査 PASS、全3 split の生成物検査 PASS、deployment API で fixed deployment 4 本の同期を確認。
- **残作業**: 実際の Chat 配信は、設定を有効化後に操作者が管理するテスト操作で 1 回確認する。
- 詳細は `docs/278_RELEASE_STATE_v376.92_2026-09-12.md`。

---

## v376.91 — 2026-09-10 🐛 公開ポータルのコントラスト修正（全3split @400×2 / @158 / @255）

公開 a11y 回帰で、退会カードの補助ラベルに白文字とのコントラスト不足が検出された。背景を `amber-700` に変更し、WCAG 2.2 AA の必要コントラストを満たすようにした。

- **影響範囲**: 公開ポータルの補助ラベル表示のみ。スキーマ・認証・業務データ・退会の業務フローは変更なし。
- **検証**: prerelease PASS。公開 a11y 3 view は critical / serious / moderate / minor すべて 0。公開 responsive は 320〜1920px の 7 viewport PASS。
- 詳細は `docs/277_RELEASE_STATE_v376.91_2026-09-10.md`。

---

## v376.90 — 2026-09-10 🐛 公開本人確認の生成物欠落を修正（全3split @399×2 / @157 / @254）

退会・会員情報変更の本人確認で、公開用の build pruner が設定オブジェクトと正規化関数を除外し、照合前に内部例外となる障害を修正した。設定を到達可能な関数へ移し、公開画面は内部例外を表示せず日本語の安全な案内へ変換する。

- **再発防止**: 公開 `Code.gs` に本人確認設定と正規化関数が残ること、内部エラー名を画面へ出さないこと、公開生成物から抽出した本人確認の成功／不一致失敗を `test:public-identity` に追加。
- **影響範囲**: 本人確認の実行経路だけ。スキーマ・認証方式・業務データ・退会申請の書込み仕様は変更なし。
- **検証**: prerelease PASS、公開 responsive 320〜1920px の 7 viewport PASS。
- 詳細は `docs/276_RELEASE_STATE_v376.90_2026-09-10.md`。

---

## v376.89 — 2026-09-09 🔒 U-27：旧公開 API の撤去（全3split @398×2 / @156 / @253）

v264 で公開ポータルの本人確認を作り直した後も、旧 v261 系の公開 API 6 本が画面から切り離されたまま外部呼出し可能だった。旧経路は名義照合・試行回数制限・現行の変更申請キューを経由しないため、公開入口から完全に撤去した。

- **撤去**: 公開許可リスト、`processApiRequest` の分岐、実装本体から旧 action 6 本を削除。
- **既存業務の維持**: 承認済み変更申請に必要な職員追加は内部専用 `addApprovedStaffMember_` へ分離。公開入口は現行の本人確認と変更申請キューだけに統一。
- **再発防止**: `test:action-dispatch` が廃止 action の許可リスト・dispatch・関数定義への残存を検出する。
- **E2E ハーネス修正**: 公開ポータルの設定読込み完了を待つようにし、v376.74 で廃止された重要事項モーダルではなく現行の注意事項ステップを検査対象に更新。
- **検証**: prerelease 31 suites PASS。deployment API で fixed deployment 4 本の同期を確認。公開 a11y 3 view は違反 0、公開 responsive 7 viewport、管理 responsive 7 viewport × 8 console はすべて fatal / console error / 横スクロール / 24px未満の操作対象 0。
- 詳細は `docs/archive/release_history/275_RELEASE_STATE_v376.89_2026-09-09.md`。

---

## v376.71 — 2026-09-04 🔒 ログイン失敗の時限解除（全3split @380×2 / @139 / @236）

**セキュリティ改善。** 会員ログインは 5 回失敗で無期限ロックだった。ログイン ID は介護支援専門員番号で
名簿から分かるため、第三者が狙った相手を無期限に締め出せる状態だった。

- **連続失敗のみを数え、成功した時点で 0 に戻す**。3 回目→1 分 / 4 回目→5 分 / 5 回目→15 分 /
  6 回目以降→60 分（上限）。連続 20 回で管理者対応が必要な恒久ロック。
- **ロック中もパスワード不一致と同一文言**にし、ロック状態を攻撃者に教えない。
  ロック中は正しいパスワードでも通さない（待ち時間の短縮を許さない）。
- **閾値のベタ書き 2 か所を撤去**して `LOGIN_LOCKOUT_POLICY` に集約（`AGENTS.md` §3）。
- **スキーマ変更**: `T_認証アカウント` に `ロック解除予定日時` を追加。operator が
  `runRebuildSchemaForV376_71` を実行して移行済み（19 列・`criticalFailed=0`）。
- **移行前に無期限ロックされた行は解除対象**として扱い、締め出しを残さない。
- **検証**: prerelease 19 スイート PASS（新ゲート `test:login-lockout` 14 件）。
  `dryRunLoginLockoutV376_71_LOG` 11/11 PASS。**実機で「3 回失敗 → ロック中は正しいパスワードでも拒否 →
  待機後にログイン成功」までを確認**。公開 a11y 0・公開 responsive PASS・管理 56 view PASS。
- **あわせてゲートを修正**: 公開側の responsive も fatal で exit 0 を返していた（v376.70 で admin / member は
  修正済み・public が残っていた）。fatal があれば exit 1 で落とすようにした。
- 詳細は `docs/265_RELEASE_STATE_v376.71_2026-09-04.md`。

---

## v376.70 — 2026-09-04 🆕 仕様書の一本化に伴う UI 不整合の修正（全3split @379×2 / @138 / @235）

仕様書を `docs/spec/` の 5 文書へ巻き直す過程で見つかった 3 件を修正した。機能追加もスキーマ変更も無い。

- **`View` 型に `'data-export'` を追加**。`renderContent()` は分岐を積み上げる構造で、`'data-export'` の判定に
  到達する時点で `currentView` が `never` に絞られており、**型検査をすり抜けていた**。
- **パンくずに `'data-export'` を追加**。データ出力（CSV）の画面だけパンくずが出ていなかった。
- **ログイン画面の未使用の英語文言変数を削除**（`AGENTS.md` §4.4）。
...
## v376.69 — 2026-09-03 🆕 一括編集の対象拡張（連絡先・勤務先・自宅）（全3split @378×2 / @137 / @234）

GCP 移行の前提機能 B（`docs/261` T-07）。一括編集で直せるのは 6 項目だけで、住所や電話をまとめて直すには
スプレッドシートを直接開くしかなかった。移行後はそれができなくなるため対象を広げた。

- **追加 14 項目**: 勤務先電話 / FAX / 携帯 ／ 事業所名・郵便番号・都道府県・市区町村・住所・建物名 ／
  自宅の郵便番号・都道府県・市区町村・住所・建物名（計 20 項目）。
- **UI**: 列が増えすぎないよう折りたたみ式（「連絡先・住所も編集する」で展開）。
- **事業所職員は対象外**: `T_事業所職員` は電話・住所の列を持たない（連絡先は所属事業所＝`T_会員` が正本）。
  保存先が無いものを許可リストに入れない、という判断を unit test で固定した。
- **意図的に対象外**: 氏名・カナ・介護支援専門員番号・会員種別・職員数上限（本人特定とログイン ID に直結）。
- **検証**: prerelease 全ゲート PASS（新ゲート `test:batch-edit` 7 件）。live は本番 API が拡張 7 項目を返し
  職員行には含まれないことを実測（345 件）。admin responsive 56 view／メール設定 E2E 5/5／公開 a11y 0・21 view。
- 詳細は `docs/263_RELEASE_STATE_v376.69_2026-09-03.md`。
