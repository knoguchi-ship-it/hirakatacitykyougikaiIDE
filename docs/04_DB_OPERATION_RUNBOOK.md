...
- 以下のシートが存在すること（Code.gs `マスタ定義` / `テーブル定義` に準拠）
  ...
  - メインDBテーブルシート: `T_会員`, `T_システム設定`, `T_事業所職員`, `T_認証アカウント`, `T_管理者Googleホワイトリスト`, `T_画面項目権限`, `T_研修`, `T_研修申込`, `T_年会費納入履歴`, `T_年会費更新履歴`, `T_外部申込者`, `T_会員_archive`, `T_事業所職員_archive`, `T_変更申請`, `T_人物統合ログ`, `T_役員`, `T_振込口座`, `T_支払い`, `T_支払い明細`, `T_請求`
...
2. 更新
  ...
  - 金額は `M_会員種別.年会費金額` から自動表示され、年会費コンソールの画面では編集できないこと（金額の変更は 設定 → 会費設定）
...
## 5.7 同一人物移行・変更申請キューの運用検証（v335）

1. スキーマ
  - `M_会員状態` に `TRANSFERRED`（移行済み）が存在すること。
  - `T_会員` に `移行日` が存在すること。
  - `T_人物統合ログ` が存在すること。
2. 公開ポータル
  - 新規入会申込は即時に `T_会員` を作成せず、`T_変更申請.申請種別コード=MEMBER_APPLICATION` / `申請状態コード=PENDING` として記録されること。
  - 申込完了画面には会員番号ではなく受付番号（申請ID）が表示されること。
3. 承認
  - 管理者が変更申請管理で承認すると、既存の入会登録処理が実行され、承認結果が画面に表示されること。
  - `介護支援専門員番号` が既存の個人/賛助会員または事業所職員と一致する場合、旧レコードを `TRANSFERRED` または `LEFT` とし、認証・役員・請求・振込口座・研修申込などの関連レコードが新主体へ移行すること。
  - `介護支援専門員番号` が空の人物は自動統合対象にせず、重複を許容すること。
4. 修復
  - データ管理コンソールの会員CM番号重複修復は、旧会員を `TRANSFERRED` にし、移行日と `T_人物統合ログ` を記録すること。

## 5.8 会員ステータスメモの運用検証（v340）

1. スキーマ
  - `T_会員` に `ステータスメモ` が存在すること。
  - `ステータスメモ` は `T_会員` の末尾列として追加され、既存列の値がずれていないこと。
  - `T_システム設定.DB_SCHEMA_VERSION` が `2026-05-12-member-status-note-v1` 以上の現行値であること。
2. 管理者コンソール
  - 会員詳細のステータス欄に `ステータスメモ（管理者のみ）` が表示されること。
  - 2,000文字以内で保存でき、再読込後も保持されること。
  - ステータスメモ変更時に監査ログへ変更内容が記録されること。
3. 会員・公開境界
  - 会員マイページに `ステータスメモ` が表示されないこと。
  - 公開ポータルの申込・変更申請・本人確認系レスポンスに `statusNote` が含まれないこと。
4. 既存ステータス運用
  - 会員状態、入会日、退会日、退会処理日、中途退会の既存編集が従来どおり保存できること。
