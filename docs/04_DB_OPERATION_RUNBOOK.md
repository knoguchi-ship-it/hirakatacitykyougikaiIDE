...
- 以下のシートが存在すること（Code.gs `マスタ定義` / `テーブル定義` に準拠）
  - マスタシート: `M_会員種別`, `M_会員状態`, `M_発送方法`, `M_郵送先区分`, `M_職員権限`, `M_職員状態`, `M_システムロール`, `M_研修状態`, `M_申込状態`, `M_会費納入状態`
  - テーブルシート: `T_会員`, `T_システム設定`, `T_事業所職員`, `T_認証アカウント`, `T_ログイン履歴`, `T_管理者Googleホワイトリスト`, `T_画面項目権限`, `T_研修`, `T_研修申込`, `T_年会費納入履歴`
