...
| 5 | **build の関数抽出パーサが二重引用符入り正規表現で壊れる**（v376.68 で実発生） | `collectFunctionDeclarationsShared`（3 ファイルに複製）が `/"/g` のような regex を文字列開始と誤認し、**以降のすべての関数を検出できなくなる**。v376.68 では operator ツール 4 件が生成物から消え、ビルドが落ちて発覚した。回避としてソース側で regex を使わない実装にしたが、**パーサ自体は未修正**。次に同じ書き方をすれば再発する |
| 4 | **gas-src 内の介護支援専門員番号 regex の重複**（2026-09-03 追記） | 名前付き定数 `CM_NUMBER_STRICT_RE_`（`gas-src:16620`）が既にあるのに、同じ `/^\d{8}$/` が `11469` / `16659` / `16724` にインラインで残っている。`AGENTS.md` §3 は「GAS 側は build pruner の罠を避けるため各関数内ローカル」を許容しているが、**同一ファイル内に定数があるのに使っていない**のは単なる重複。定数へ寄せるだけで解消できるが、認証・本人確認の経路に触れるため単独リリースで扱いたい |
