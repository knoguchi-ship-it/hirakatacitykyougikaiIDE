...
| 用途 | プロジェクト | Deployment ID（省略） | URL | アクセス制御 |
|---|---|---|---|---|
...
> v251〜: `doGet` は `ScriptApp.getScriptId()` で配信ページを固定。`?app=` パラメータは無効。

Project rule:

- **統合プロジェクト（scriptId: `11YRlyWVgW...`）は公開ポータル専用**。会員・管理者機能は提供しない。
- **3プロジェクト（public / member / admin）は物理的に完全分離**（v251〜確定）。
- 統合プロジェクトの固定 deployment 2 本（`AKfycbyw...` / `AKfycbxy...`）は公開ポータルとして継続維持。

## 2. (B) 分離プロジェクト Deployment IDs（本番）
...
- Verification:
  ...
  - `npm run build:gas` ✅ / `npm run build:gas:member` ✅ / `npm run build:gas:admin` ✅
  - 統合 push ✅（5 files）/ 会員 split push ✅（3 files）/ 管理者 split push ✅（3 files）
  - 統合 `npx clasp redeploy` ✅（`AKfycbyw... @255` / `AKfycbxy... @255`）
  - 会員 split `npx clasp redeploy` ✅（`AKfycbxd... @8`）
  - 管理者 split `npx clasp redeploy` ✅（`AKfycbwS... @12`）
  - `npx clasp deployments --json` ✅
  ...
  - 実ブラウザ確認 → 操作者側で実施（確認ポイントは `docs/126` / `docs/127` 参照）
...
- Scope: 公開ポータル切り分け完了・3プロジェクト分離確定。
  ...
  - `MEMBER_PORTAL_URL`: 会員 split URL (`AKfycbxd.../exec`) に更新。認証情報メールのリンク先が変更済み。
- 対象 deployment: 統合（公開）2本 / 会員 split / 管理者 split — 全 4 deployment 更新。
- Verification:
  - `npm run build:gas` ✅ / `npm run build:gas:member` ✅ / `npm run build:gas:admin` ✅
  - 統合 push ✅（5 files）/ 会員 split push ✅（3 files）/ 管理者 split push ✅（3 files）
  - 統合 `npx clasp redeploy` ✅（`AKfycbyw... @250` / `AKfycbxy... @250`）
  - 会員 split `npx clasp redeploy` ✅（`AKfycbxd... @3`）
  - 管理者 split `npx clasp redeploy` ✅（`AKfycbwS... @4`）
  - 実ブラウザ確認 → 操作者側で実施（確認ポイントは `docs/117` 参照）
