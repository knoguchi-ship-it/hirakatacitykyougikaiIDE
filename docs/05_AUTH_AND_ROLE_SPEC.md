...
- 左メニューの表示名と会員マイページ種別は、`T_管理者Googleホワイトリスト` の `紐付け認証ID` / `紐付け会員ID` から解決した会員・職員情報を優先表示し、`システム管理者` は紐付け未解決時のフォールバック表示とする。
- `T_管理者Googleホワイトリスト` の追加・更新・削除は、管理者ログイン後の `管理コンソール（システム権限）` で運用する。

4. 管理コンソール アクセス権限（2026-03-13 追加）
- `T_管理者Googleホワイトリスト` に `有効フラグ=true` で登録されたアカウントは、**会員管理コンソール・年会費管理コンソール・研修管理コンソール・管理コンソール（システム権限）・システム設定** の全てにアクセスできる（同一権限）。
- 両コンソール間は双方向ナビゲーションで遷移可能。再認証は不要（セッション継続中）。
- 会員ログイン（ID/PW）のみのアカウントには、コンソール間ナビゲーションリンクを表示しない。
- 認証判定は各管理コンソールで共通して `checkAdminBySession_()` を使用する。
- `管理コンソール（システム権限）` では、`Googleメール` / `GoogleユーザーID` / `表示名` / `紐付け認証ID` / `有効フラグ` を管理し、表示対象の会員・職員は `紐付け認証ID` から解決する。
