> **【アーカイブ】この文書は 2026-09-04 に役目を終えた。正本は docs/spec/03_TRD.md。**
...
- **年会費管理コンソール**:
  ...
  - 旧年度の実績が存在しても、会員マイページでは current / previous fiscal year の連続表示を優先し、2024年度の実績で 2025年度欠損を見えにくくしない。
...
- **郵便番号入力UI**:
  - 画面上の郵便番号入力は 3 桁 + 4 桁の2入力を標準とする。
  - フロントエンド内部の値、および `T_会員` への保存値は従来どおり `123-4567` の単一文字列を維持する。
  - 共通コンポーネントで `inputmode="numeric"`、自動フォーカス移動、貼り付け正規化、`autocomplete="postal-code"` を統一適用する。
...
### 公開入会申込の遷移ルール
- 判定キーは `介護支援専門員番号` を正本とし、事業所会員本体の重複判定には `事業所番号` を併用する。
- 個人会員申込時に、同じ `介護支援専門員番号` の在籍中事業所メンバーが存在する場合は、その事業所メンバーを退会処理し、個人会員へ切り替える。
- 事業所会員申込時に、同じ `介護支援専門員番号` の個人会員または賛助会員が存在する場合は、その個人系会員を退会処理し、申込先事業所メンバーとして登録する。
- 事業所会員申込時に、同じ `介護支援専門員番号` の別事業所在籍メンバーが存在する場合は、元事業所メンバーを退会処理し、申込先事業所メンバーへ転籍する。
- 上記以外の新規入会で同じ `介護支援専門員番号` または同じ `事業所番号` が既に有効登録済みの場合は、二重登録として受付を拒否する。
- 代表者が転出する場合は、残留メンバーから後任代表者を自動選定し、残留者がいない場合は元事業所会員を退会状態へ更新する。
...
- 元データ: `T_年会費納入履歴`
- 監査データ: `T_年会費更新履歴`
- 金額マスタ: `M_会員種別.年会費金額`（v376.64〜 管理画面 設定 → 会費設定 から変更可能。年会費コンソール側の画面入力では従来どおり変更させない）
