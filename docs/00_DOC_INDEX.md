# Document Index

Updated: 2026-04-13 (v203)

## 1. Entry Sources Of Truth
- `HANDOVER.md`
- `AGENTS.md`
- `GLOBAL_GROUND_RULES/docs/AI_RULES/05_PROJECT_RULES_HIRAKATA.md`
- `GLOBAL_GROUND_RULES/docs/AI_RULES/00_OPERATING_MODEL.md`
- `GLOBAL_GROUND_RULES/docs/AI_RULES/10_WORKFLOW_AND_QUALITY.md`
- `GLOBAL_GROUND_RULES/docs/AI_RULES/20_SECURITY_APPROVALS.md`
- `GLOBAL_GROUND_RULES/docs/AI_RULES/30_ERROR_MEMORY.md`
- `GLOBAL_GROUND_RULES/docs/AI_RULES/40_DOCS_AND_TEACHING.md`
- `docs/44_DEVELOPMENT_HANDOVER_PLAYBOOK_2026-04-04.md`
- Latest release-state document referenced from `HANDOVER.md`
- `docs/09_DEPLOYMENT_POLICY.md`
- `docs/05_AUTH_AND_ROLE_SPEC.md`
- `docs/04_DB_OPERATION_RUNBOOK.md`
- `docs/03_DATA_MODEL.md`
- `CLAUDE.md` is legacy-only; `AGENTS.md` is the canonical entry point

## 2. Functional Sources Of Truth
- `docs/01_PRD.md`
- `docs/02_ARCHITECTURE.md`
- `docs/03_DATA_MODEL.md`
- `docs/04_DB_OPERATION_RUNBOOK.md`
- `docs/05_AUTH_AND_ROLE_SPEC.md`
- `docs/11_WITHDRAWAL_DELETION_POLICY.md`

## 3. Ops And Incident Docs
- `docs/73_RELEASE_STATE_v203_2026-04-13.md` - **v203 current** - user-facing quick-start intro copy
- `docs/72_RELEASE_STATE_v202_2026-04-13.md` - v202 record - removed redundant roster help button
- `docs/71_RELEASE_STATE_v201_2026-04-13.md` - v201 record - plain-language cleanup for remaining template-help sections
- `docs/70_RELEASE_STATE_v200_2026-04-12.md` - v200 record - plain-language template help page
- `docs/69_RELEASE_STATE_v199_2026-04-12.md` - v199 record - sample template link on help page
- `docs/68_RELEASE_STATE_v198_2026-04-12.md` - v198 record - dedicated template help page + template validation
- `docs/67_RELEASE_STATE_v197_2026-04-11.md` - v197 record - template help dialog + roster/reminder template coexistence
- `docs/66_ROSTER_TEMPLATE_GUIDE_2026-04-10.md`
- `docs/65_HANDOVER_TASK_DEV_SETUP_2026-04-10.md`
- `docs/64_RELEASE_STATE_v194_v196_2026-04-10.md`
- `docs/63_SOW_ROSTER_PDF_AND_BULK_MAIL_2026-04-10.md`
- `docs/62_RELEASE_STATE_v193_2026-04-09.md`
- `docs/61_RELEASE_STATE_v191_2026-04-09.md`
- `docs/60_RELEASE_STATE_v189_2026-04-09.md`
- `docs/59_RELEASE_STATE_v188_2026-04-09.md`
- `docs/58_NEXT_TASK_PERFORMANCE_2026-04-09.md`
- `docs/57_RELEASE_STATE_v187_2026-04-09.md`
- `docs/56_RELEASE_STATE_v186_2026-04-09.md`
- `docs/55_RELEASE_STATE_v179_v185_2026-04-09.md`
- `docs/54_HANDOVER_TASK_v177_BROWSER_CONFIRM_2026-04-09.md`
- `docs/53_RELEASE_STATE_v178_2026-04-09.md`
- `docs/52_RELEASE_STATE_v177_2026-04-08.md`
- `docs/51_RELEASE_STATE_v176_2026-04-08.md`
- `docs/50_RELEASE_STATE_v175_2026-04-08.md`
- `docs/49_RELEASE_STATE_v174_2026-04-08.md`
- `docs/48_RELEASE_STATE_v173_2026-04-07.md`
- `docs/47_RELEASE_STATE_v172_2026-04-07.md`
- `docs/46_RELEASE_STATE_v171_2026-04-06.md`
- `docs/45_RELEASE_STATE_v170_2026-04-04.md`
- `docs/44_DEVELOPMENT_HANDOVER_PLAYBOOK_2026-04-04.md`
- `docs/40_HANDOVER_TASK_BUSINESS_ADMIN_ROLE_CHANGE_RULE.md`
- `docs/39_IMPLEMENTATION_BEST_PRACTICES_2026-03-31.md`
- `docs/38_HANDOVER_TASK_v155_PRIVACY_AND_DEPLOYMENT_CONFIRM.md`
- `docs/37_GAS_QUOTAS_AND_LIMITS.md`
- `docs/36_DATA_PROTECTION_PROCEDURES.md`
- `docs/35_BEST_PRACTICE_AUDIT_2026-03-31.md`
- `docs/34_DEVELOPMENT_OPERATING_MODEL_2026-03-30.md`
- `docs/32_HANDOVER_TASK_v142_RETEST_AND_DEPLOYMENT_CONFIRM.md`
- `docs/31_HANDOVER_TASK_TEMPLATE.md`
- `docs/17_ROOT_CAUSE_ERROR_RESPONSE_PLAYBOOK.md`
- `docs/16_INCIDENT_clasp_run_permission_2026-03-14.md`

## 4. Handover Tasks
- Template: `docs/31_HANDOVER_TASK_TEMPLATE.md`
- Active performance follow-up: `docs/58_NEXT_TASK_PERFORMANCE_2026-04-09.md`
- Current setup verification: `docs/65_HANDOVER_TASK_DEV_SETUP_2026-04-10.md`
- Active rule task: `docs/40_HANDOVER_TASK_BUSINESS_ADMIN_ROLE_CHANGE_RULE.md`
- Browser confirmation record: `docs/54_HANDOVER_TASK_v177_BROWSER_CONFIRM_2026-04-09.md`
- Completed: `docs/32_HANDOVER_TASK_v142_RETEST_AND_DEPLOYMENT_CONFIRM.md`
- Completed: `docs/38_HANDOVER_TASK_v155_PRIVACY_AND_DEPLOYMENT_CONFIRM.md`
- Completed: `docs/53_HANDOVER_TASK_v177_ALIGNMENT_AUDIT_2026-04-08.md`

## 5. Major Decision And Spec Records
- `docs/06_DECISION_RECORD_AUTH_2026-02-28.md`
- `docs/07_DECISION_RECORD_PUBLIC_PORTAL_2026-03-13.md`
- `docs/13_DECISION_RECORD_MAIL_CONSOLE_2026-03-13.md`
- `docs/18_DECISION_RECORD_ANNUAL_FEE_CONSOLE_2026-03-15.md`
- `docs/19_DECISION_RECORD_PUBLIC_PORTAL_APPLICATION_INTEGRATION_2026-03-17.md`
- `docs/21_IMPL_SPEC_FIELD_ACCESS_CONTROL_v106.md`
- `docs/22_TEST_SPEC_v106_FIELD_ACCESS_CONTROL.md`
- `docs/23_MIGRATION_HANDOVER_v128.md`
- `docs/24_TABLE_DESIGN_MIGRATION_v128_REDO_2026-03-25.md`
- `docs/25_SOW_MIGRATION_v128_REDO_2026-03-25.md`
- `docs/27_IMPL_SPEC_ADMIN_DASHBOARD_AND_OFFICE_MEMBER_v131.md`
- `docs/29_IMPL_SPEC_INLINE_STAFF_EDIT_v136_v140.md`
- `docs/30_TEST_SPEC_v136_v140_INLINE_STAFF_EDIT.md`

## 6. Reference Material Only
- `docs/learning/index.html`
- `docs/learning/07_system_overview_v151.html`
- `docs/learning/08_system_overview_v154.html`

## 7. Rules
- Update canonical docs in the same turn as code or deployment changes.
- Record task documents as handover artifacts, not as substitutes for canonical specs.
- `docs/learning/` is reference-only and never the source of truth.
- Archive or replace obsolete entry points instead of leaving them ambiguous.
