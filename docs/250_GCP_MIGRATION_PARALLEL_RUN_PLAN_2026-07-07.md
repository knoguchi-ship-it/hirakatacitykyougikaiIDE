...
> **【2026-09-03 operator 決定】GCP 移行作業は一旦中断**。当面の新規仕様は本番リポジトリ（GAS）側で実装する。
> 本計画は**破棄しない**（再開前提）。中断中も §6.1 の未移行 write 棚卸しは最新に保つこと。
> あわせて、これ以降に実装する仕様には **GCP 移植可能性ゲート**（`AGENTS.md` §4.8・GCP で実装できない仕様は採用しない）が適用される。
> 決定の背景は `docs/06_DECISION_RECORDS.md`「GCP 移行の一旦中断と『GCP 移植可能性ゲート』（2026-09-03）」。
