...
版数: 1.7 ／ 初版: 2026-09-03 ／ 最終更新: 2026-09-22
...
| 版 | 日付 | 変更内容 |
|---|---|---|
| 1.7 | 2026-09-22 | 差し込みは一覧表示をやめ、カードのボタンで挿入する規約を追加。表示はメール種別が持つタグだけに限る |
| 1.6 | 2026-09-22 | メール通知の内容を系統選択式にする規約を追加。「メール配信」は独立したまま残す |
| 1.5 | 2026-09-20 | メール通知設定カードのカテゴリ配色規約を追加（v376.96）。配色の正本は `src/shared/mailCategories.ts` |
| 1.4 | 2026-09-10 | SCR-08 に退会方式選択と、5項目の設定可能な確認枠を追加 |
...
| SCR-07 | 会員情報の変更申請 | 会員が住所などの変更を申請する（事務局が承認）。**個人・事業所・賛助の 3 種別**に対応 | `member-update` / `update` | SCR-01・直リンク | 完了表示（同一画面内） |
| SCR-08 | 退会申請 | 退会を申請する（事務局が承認）。**個人・事業所・賛助の 3 種別**に対応 | `withdrawal-request` / `withdraw` | SCR-01・直リンク | 完了表示（同一画面内） |

### SCR-07 / SCR-08 の共通ステップ「本人確認」

変更申請と退会申請は、**会員種別の選択と本人確認を同じ画面部品で行う**
（`src/public-portal/components/IdentityVerifyStep.tsx`）。
以前は 2 画面が同じ内容を別々に持っていたため、種別を増やしたときに片方だけ直す事故が起きた
（賛助会員が両方から漏れていた）。

画面の構成は 3 種別で共通で、種別による違いは **選べる番号の顔ぶれ** だけに閉じている。

1. 種別を選ぶ（個人 / 事業所 / 賛助。カードに「何で確認するか」を書く）
2. 名義を入れる（個人・賛助は姓と名、事業所は事業所名）
3. **番号の種類をボタンで選び、その 1 欄だけに入力する**
4. 返信用メールアドレスを入れる

番号を「1 つ選ばせる」形にしたのは、登録状況にばらつきがあるため（RD UC-007）。
入力欄を同時に複数出すと、どれを埋めればよいのかが伝わらず、
サーバー側の「ちょうど 1 つ」という条件とも食い違う。

規則の正本は `src/shared/publicIdentity.ts`。文言（種別カードの説明・入力欄の補足）もここが正本で、
画面側にベタ書きしない。
...
本人確認後に、**年度末退会**と**即時退会**をカードから一方選ぶ。選択を変えた場合は確認同意を外し、選択中の方式に対応する内容を確認枠に再表示する。

- 年度末退会: 当年度末（3月31日）に適用。予定日までは会員マイページを利用でき、年度末前なら会員マイページから取り消せる
- 即時退会: 管理者の承認日に適用。承認後は会員マイページを利用・取り消しできない
- 確認枠の5項目（退会方式／適用タイミング／マイページ利用／取消／承認）は、管理ポータルの「システム設定 → 公開ポータル」で**項目ごとに表示・非表示、項目名、年度末退会時の説明、即時退会時の説明**を編集できる
- 5項目は手続きに必要な固定項目とする。任意行の追加・削除は、説明責任と審査フローを別途設計するまで実装しない
...
基本設定／規程・重要事項／会費設定／帳票出力／メール通知／Google Chat／公開ポータル／マスタ管理。

メール通知では、停止／通常送信／テスト集約を唯一の「配信状態」とし、全体停止スイッチや送信抑止を別設定として置かない。
会員手続き、入会・登録、研修・認証などのメール種別と文面は同じ画面内で分け、Google Chatの設定は混在させない。

メール通知は、配信状態を選ぶ「メール配信」と、個別の通知を設定する「メール通知の内容」に分ける。
「メール通知の内容」は 14 枚のカードを一度に並べず、**系統を選ぶボタンを先に置き、選んだ系統のカードだけを表示する**。
ボタンには系統名と「n 件中 m 件が有効」を出し、開かなくても停止中の系統が分かるようにする。
系統を切り替えても入力中の値は保持し、保存は画面下の一括保存でまとめて行う。
送信元アドレスは全系統に共通なので常時表示する。入会・登録メールの共通スイッチは、それが効く入会と職員追加の系統でだけ表示する。

差し込み（マージタグ）は一覧を読ませず、**カード内のボタンを押して件名・本文のカーソル位置へ挿入する**。
表示するタグは、そのカードのメール種別が持つものだけとする。グループ単位で和集合を出さない
（種別で使えないタグを入れさせないため）。説明はタグ名が意味を含まないときだけ添える。

メール通知の設定カードは、カテゴリ（入会／職員追加／変更申請ワークフロー／研修／認証・セキュリティ／会員手続き）ごとに
色相とアイコンで区別する。有効・無効は色の有無で示し、無効のカードは色を抜いて「停止中」と表示する。
色だけに依存させず、アイコンの形とバッジ文言を併用する（WCAG 1.4.1）。配色の正本は `src/shared/mailCategories.ts`
であり、画面ごとにローカル定義しない。
...
## 12. Google Chat 通知設定（管理画面）

- 「システム設定 ＞ Google Chat」に「会員手続き通知」を置く。メール通知の画面には混在させない。
- 全体、受付、確定、要確認を個別に有効／無効にできる。初期状態は全体無効（安全停止）とする。
- 受付・確定・要確認の本文をそれぞれ編集でき、許可済みの差し込みタグを画面内に表示する。
- Webhook URL の入力欄・表示欄・テスト送信ボタンは置かない。接続は秘密設定で既に管理されており、通常の設定画面には接続設定を出さない。
- 受付と処理完了は同じ申請IDのスレッドにまとめることを画面に明示する。
- 360px 幅でも、トグルと本文入力欄は縦積みで操作できること。
