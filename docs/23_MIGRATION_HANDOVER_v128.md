...
**現状**:
- `T_管理者Googleホワイトリスト` には `k.noguchi@uguisunosato.or.jp` が MASTER 権限で登録済み
- しかし `紐付け会員ID` が設定されていない
- 移行データに「野口　健太」が存在しない（問題1の取りこぼしか、そもそもソース名簿に非掲載の可能性）

**対応手順**:
1. ソーススプレッドシートで「野口」を検索し、名簿上の所在を確認
2. 名簿にいる場合: 移行修正で正しく取り込み、生成された会員IDを `T_管理者Googleホワイトリスト` の `紐付け会員ID` に設定
3. 名簿にいない場合: T_会員に手動で会員レコードを作成し、紐付ける
4. いずれの場合も `T_認証アカウント` にも対応するアカウントが必要
