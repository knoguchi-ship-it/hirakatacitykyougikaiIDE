...
- ブランチ: `main`
- 最新本番反映: `v145`（事業所番号投入 + 年会費ナビ + サイドバー修正 + バリデーション緩和）。固定 deployment @145 同期済み
...
- 本番固定 Deployment:
  - 会員マイページ: `AKfycbywpWoYxij6A-ZunIeBjG1Q8qX78PMMTsT3frx1cM5PJ2nAuZpz81KruXb5LIvWgbQx` → `@145`
  - 公開ポータル: `AKfycbxyuUXgK1oHUDMahQjluiL-gcrMK0qV0FWLFYaYBqGxlRSg9NhvmbyQRyf0dvaqg7Zp` → `@145`
  - Apps Script UI の説明名: `会員メニュー (prod) v145 office number + annual fee nav + sidebar fix` / `公開ポータル (prod) v145 office number + annual fee nav + sidebar fix`
...
## 3. ~~次の最優先タスク: 残件再検証~~ → **完了（2026-03-28）**

### 概要

初回テストで FAIL / 未再検証扱いとなった以下の残件を回収する。→ **全 8 ケース PASS（2026-03-28, @144）**
...
補助 task:
- `docs/32_HANDOVER_TASK_v142_RETEST_AND_DEPLOYMENT_CONFIRM.md` — 完了。evidence 記録済み。
