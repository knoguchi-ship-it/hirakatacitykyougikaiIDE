...
- ブランチ: `main`
- 最新本番反映: `v145`（事業所番号投入 + 年会費ナビ + サイドバー修正 + バリデーション緩和）。固定 deployment は @144 → @145 更新が必要
...
- 本番固定 Deployment:
  - 会員マイページ: `AKfycbywpWoYxij6A-ZunIeBjG1Q8qX78PMMTsT3frx1cM5PJ2nAuZpz81KruXb5LIvWgbQx` → `@144`（@145 へ要更新）
  - 公開ポータル: `AKfycbxyuUXgK1oHUDMahQjluiL-gcrMK0qV0FWLFYaYBqGxlRSg9NhvmbyQRyf0dvaqg7Zp` → `@144`（@145 へ要更新）
  - Apps Script UI の説明名: 更新後 `会員メニュー (prod) v145 ...` / `公開ポータル (prod) v145 ...` とすること
