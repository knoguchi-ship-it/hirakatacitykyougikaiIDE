# ClaudeCode 次作業指示書（2026-03-29 更新）
...
- ブランチ: `main`
- 最新本番反映: `v146`（事業所番号APIマッピング修正 + 除籍時権限強制降格）。固定 deployment @146 同期済み
- GAS Version 151 作成済み。v147〜v151 は clasp push 済み・Git 未コミット。固定 deployment の @151 同期はユーザー作業待ち
...
- 本番固定 Deployment:
  - 会員マイページ: `AKfycbywpWoYxij6A-ZunIeBjG1Q8qX78PMMTsT3frx1cM5PJ2nAuZpz81KruXb5LIvWgbQx` → `@146`（@151 同期待ち）
  - 公開ポータル: `AKfycbxyuUXgK1oHUDMahQjluiL-gcrMK0qV0FWLFYaYBqGxlRSg9NhvmbyQRyf0dvaqg7Zp` → `@146`（@151 同期待ち）
