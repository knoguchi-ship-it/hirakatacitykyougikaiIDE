...
- ブランチ: `main`
- 最新本番反映: `v117`（管理者権限コンソール候補保持修正）
- GitHub のコミット/Push 状態は作業終了時点の `git log -1 --oneline` / `git status --short` を正とする
- 作業ツリーは変更が入りうるため、再開時に必ず `git status --short` を確認する
- 本番固定 Deployment:
  - 会員マイページ: `AKfycbywpWoYxij6A-ZunIeBjG1Q8qX78PMMTsT3frx1cM5PJ2nAuZpz81KruXb5LIvWgbQx` → `@117`
  - 公開ポータル: `AKfycbxyuUXgK1oHUDMahQjluiL-gcrMK0qV0FWLFYaYBqGxlRSg9NhvmbyQRyf0dvaqg7Zp` → `@117`
- v106 バックエンドテスト: B-01〜B-10 全10件 PASS（2026-03-20）
- v106 フロントエンドテスト: F-01〜F-12 全11件 PASS + 1件 SKIP（2026-03-20、@108）
- v117 デプロイチェック: `build:gas` / `healthCheck` / `getDbInfo` を通し、固定2 Deployment を `@117` へ更新。Playwright で管理者ログイン成功、左メニュー表示名 `佐藤 次郎`、`管理コンソール（システム権限）` 表示、既存管理者レコードの紐付け select 保持、公開ポータル表示、個人会員ログイン成功を確認（2026-03-22）
...
- v117: `管理コンソール（システム権限）` の紐付け候補生成を修正。既存リンク中の `事務局管理者` 認証なども候補 select から消えず、既存レコード編集時に選択値が保持されるようにした。
- v116: `T_管理者Googleホワイトリスト` を操作する `管理コンソール（システム権限）` を追加。Googleメール / GoogleユーザーID / 表示名 / 紐付け認証ID / 有効フラグの CRUD を管理画面から実施可能にした。
- v116: 管理者ログイン後の左メニュー表示名と会員マイページ種別を、ホワイトリストに紐付く会員・職員情報ベースへ変更。紐付け未解決時のみ `システム管理者` を表示する。

- v115: 個人会員住所の未使用側デフォルト値を「入力あり」と見なさないよう補正し、勤務先未入力のまま自宅側だけ入力しても形式エラーが出ないよう修正。
- v114: 個人会員の住所情報で勤務先/自宅とも `大阪府` / `枚方市` / `573-` を初期表示し、未使用側のデフォルト都道府県・郵便番号・市区町村は DB 保存前に除去するよう変更。
- v113: 新規入会申込フォームを改善。フリガナのカタカナ検証、郵便番号/事業所番号/電話番号/介護支援専門員番号の形式検証を追加し、事業所会員では `大阪府` / `枚方市` / `573-` を初期値設定、職員登録を3名初期表示へ変更。
- v112: `getMemberPortalData` を管理者専用アクションから除外し、一般会員が Google アカウント未ログインでも会員ログイン後に会員ページを開けるよう修正。
...
1. ~~**v131: 管理コンソール年度対応・事業所会員基本情報整理・入会日補正**~~ ✅ 実装完了（Version 131 作成済み、固定 Deployment @131 更新待ち）
   - ダッシュボード会計年度対応、事業所会員フィールドブランク化（DB+UI+バックエンド）、入会日補正163件、不明37件リスト作成
2. 管理コンソール UI/UX の継続改善
3. 必要なら会員一括編集（`updateMembersBatch_`）の対象項目拡張
4. 必要なら公開ポータルの文言・導線改善
