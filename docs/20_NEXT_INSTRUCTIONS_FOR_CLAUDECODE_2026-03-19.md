...
- ブランチ: `main`
- 最新本番反映: `v116`（管理者権限コンソールと管理者表示名連携）
- GitHub のコミット/Push 状態は作業終了時点の `git log -1 --oneline` / `git status --short` を正とする
- 作業ツリーは変更が入りうるため、再開時に必ず `git status --short` を確認する
- 本番固定 Deployment:
  - 会員マイページ: `AKfycbywpWoYxij6A-ZunIeBjG1Q8qX78PMMTsT3frx1cM5PJ2nAuZpz81KruXb5LIvWgbQx` → `@116`
  - 公開ポータル: `AKfycbxyuUXgK1oHUDMahQjluiL-gcrMK0qV0FWLFYaYBqGxlRSg9NhvmbyQRyf0dvaqg7Zp` → `@116`
- v106 バックエンドテスト: B-01〜B-10 全10件 PASS（2026-03-20）
- v106 フロントエンドテスト: F-01〜F-12 全11件 PASS + 1件 SKIP（2026-03-20、@108）
- v116 デプロイチェック: `typecheck` / `build` / `build:gas` / `healthCheck` / `getDbInfo` を通し、固定2 Deployment を `@116` へ更新。公開ポータル表示と個人会員ログイン成功を確認。管理者ログイン画面到達も確認したが、この Playwright セッションでは Google セッション未取得のため管理者ログイン完了までは未確認（2026-03-22）
