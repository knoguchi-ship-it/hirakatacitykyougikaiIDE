...
function enqueueMemberApplicationChangeRequest_(payload) {
  ...
  MailApp.sendEmail(
    contactEmail,
    '【枚方市介護支援専門員連絡協議会】入会申込を受け付けました',
    [
      applicantName + ' 様',
      '',
      '入会申込を受け付けました。事務局が内容を確認し、承認後に登録を反映します。',
      '',
      '申請ID: ' + requestId,
...
function assertSingleActiveAffiliationByCareManager_(ss, careManagerNumber, contextLabel) {
  var normalized = String(careManagerNumber || '').trim();
  if (!normalized) return;

  var memberRows = getRowsAsObjects_(ss, 'T_会員').filter(function(row) {
    return !toBoolean_(row['削除フラグ'])
      && String(row['介護支援専門員番号'] || '').trim() === normalized
      && String(row['会員種別コード'] || '') !== 'BUSINESS'
      && !isInactiveMemberStatusForIdentity_(row['会員状態コード']);
  });
  var staffRows = getRowsAsObjects_(ss, 'T_事業所職員').filter(function(row) {
    return !toBoolean_(row['削除フラグ'])
      && String(row['介護支援専門員番号'] || '').trim() === normalized
  ...
  if (memberRows.length > 0 && staffRows.length > 0) {
    throw new Error((contextLabel || '会員種別変換') + ' 後の整合性チェックに失敗しました。介護支援専門員番号 ' + normalized + ' に個人/賛助会員と事業所会員メンバーが同時に有効で残っています。');
  }
  if (staffRows.length > 1) {
    var dupIds = staffRows.map(function(r) { return r['職員ID']; }).join(', ');
    throw new Error((contextLabel || '会員種別変換') + ' 後の整合性チェックに失敗しました。介護支援専門員番号 ' + normalized + ' の在籍職員レコードが複数存在します（職員ID: ' + dupIds + '）。');
  }
}
...
/**
 * 初期化順序の不変条件 (v342, docs/204 再発防止):
 *   1. createMasterSheets_ は writeSheetHeaders_ の name-based shift により、
 *      master sheet の列追加でデータ列が残存していても安全に migrate される。
 *   2. ensureTableSheetsExist_ は欠落シートのみ追加。既存シートのヘッダーは触らない。
 *   3. normalizeTableColumns_ が各 T_* テーブルの name-based shift を実施する。
 *   4. auditDeleteFlagColumns_ は migration 後の sanity check (read-only) であり、
 *      schema-drift の取りこぼしを Logger.log で警告する。
 * createTableSheets_ は v340 以降 initializeSchema_ から外している。既存シートの
 * ヘッダー上書きより前に name-based migration を走らせる目的。
 */
...
function approveAdminChangeRequest_(payload) {
  ...
  // 申請者への通知
  if (contactEmail) {
    var typeLabel2 = { MEMBER_APPLICATION: '入会申込', MEMBER_UPDATE: '登録情報変更申請', WITHDRAWAL: '退会申請', STAFF_ADD: '職員追加申請', STAFF_REMOVE: '職員除籍申請' };
    MailApp.sendEmail(
      contactEmail,
...
function rejectAdminChangeRequest_(payload) {
  ...
  if (contactEmail) {
    var typeLabel3 = { MEMBER_APPLICATION: '入会申込', MEMBER_UPDATE: '登録情報変更申請', WITHDRAWAL: '退会申請', STAFF_ADD: '職員追加申請', STAFF_REMOVE: '職員除籍申請' };
    MailApp.sendEmail(
      contactEmail,
...
function backupMigrationTargetsJson() {
  return JSON.stringify(backupMigrationTargets_());
}
