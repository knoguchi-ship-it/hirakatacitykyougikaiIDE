...
更新日: 2026-05-11
現行本番: `v331`（パスワード規約を 8〜19 文字 + 許可文字セット（半角英数 + 安全記号）に変更、警告をパスワード変更モーダル内表示、`getOfficerMasterData` を `MEMBER_ALLOWED_ACTIONS` に追加して役員会員の `unsupported_action` を解消） / integrated-public GAS version `295` / member split GAS version `51` / admin split GAS version `89`
fixed deployment: integrated/public `@295` x2 / member split `@51` / admin split `@89`
...
|---|---|---|---|---|
| 公開ポータル | integrated/public | `AKfycbxyuUXgK1oHUDMahQjluiL-gcrMK0qV0FWLFYaYBqGxlRSg9NhvmbyQRyf0dvaqg7Zp` | `ANYONE_ANONYMOUS` | `@295` |
| 公開ポータル legacy | integrated/public | `AKfycbywpWoYxij6A-ZunIeBjG1Q8qX78PMMTsT3frx1cM5PJ2nAuZpz81KruXb5LIvWgbQx` | `ANYONE_ANONYMOUS` | `@295` |
| 会員マイページ | member split | `AKfycbxd_6HlH5aWLhxYOtLUHehI3ODiHg4fpc5SCzNdEBIDbDpaBuU3KTuqDRbeBmhWZxSQ_g` | `ANYONE_ANONYMOUS` | `@51` |
| 管理者ポータル | admin split | `AKfycbwSCTTyvWY_cFG764XawdbqA8r0qxYbav4aDZ-BK9rRmvXHoUXrKQnQ9egRGqWcx4Os` | `DOMAIN` | `@89` |
