...
- **本番**: public **@410×2** / member **@168** / admin **@265**（**v376.99**）。4 本を同一リリースへ切替済み。
  ロールバック先は public @409×2 / member @167 / admin @264（v376.98.1）。Apps Script API の一覧で 4 本が現行版を指すことを確認済み。
- **直近リリース（2026-09-22 / v376.99）**: ログインID変更・メールアドレス変更の通知を設定画面で編集できるようにし、固定文を解消。設定キーだけあって送信されていなかった「会員情報変更確認」「退会申請受付」の 2 枚も配線した。公開ポータルの「変更するとログインIDも変わります」は削除し、変わった事実をメールで伝える方式へ改めた。詳細は `docs/285`。
  **本番の実画面確認は未実施**（ブラウザ認証切れ）。
