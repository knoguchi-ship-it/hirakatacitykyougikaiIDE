...
更新日: **2026-03-18（v100 公開ポータル統合・公開入会導線・代表者権限制約実装）**
...
- 現在の作業ツリーは **未コミット変更あり**。次担当者は作業開始前に必ず `git status --short` を確認し、スクリーンショット類や `docs/learning/` 追加を含む現状差分を把握してから継続すること。
...
- **備考**: 公開ポータルを「枚方市介護支援専門員連絡協議会お申込みポータル」に再編。トップ画面に「研修申込」「新規入会申込」を追加し、新規入会を管理コンソールから削除して公開ポータルへ統合。`submitMemberApplication` を公開 API 化。`updateMember_` に代表者権限制約のバックエンド強制を追加。MCP で会員/公開両 URL の表示と管理画面導線削除を確認。
