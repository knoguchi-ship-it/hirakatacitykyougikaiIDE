...
## ▶ 再開プラン（2026-09-22 時点・次の担当者はここから）

### 0. 30 秒で現状

- **本番**: public **@411×2** / member **@169** / admin **@266**（**v376.100**）。4 本を同一リリースへ切替済み。
  ロールバック先は public @410×2 / member @168 / admin @265（v376.99）。Apps Script API の一覧で 4 本が現行版を指すことを確認済み。
- **直近リリース（2026-09-22 / v376.100）**: 事業所職員の介護支援専門員番号を変えるとログインIDが追従するようにし、職員本人と代表者へ通知する。既存の不一致は触らない。**実データ確認は未実施**。詳細は `docs/286`。
- **同日リリース（v376.99）**: ログインID変更・メールアドレス変更の通知を設定画面で編集できるようにし、固定文を解消。設定キーだけあって送信されていなかった「会員情報変更確認」「退会申請受付」の 2 枚も配線した。公開ポータルの「変更するとログインIDも変わります」は削除し、変わった事実をメールで伝える方式へ改めた。詳細は `docs/285`。
- **同日リリース（v376.98 / v376.98.1）**: メール通知の差し込みを一覧表示からクリック挿入のボタンへ変更。あわせて、表示タグと実際に使えるタグの不一致（事業所・メンバーのタグ未表示／職員追加・ワークフローで和集合を表示）を解消した。詳細は `docs/284`。
- **同日リリース（2026-09-22）**: 「システム設定 ＞ メール通知」の「メール通知の内容」を系統選択式にした。6 系統のボタンを先頭に置き、選んだ系統のカードだけを表示する（一度に出るカードは最大 14 枚 → 3 枚）。「メール配信」は独立したまま。表示のみの変更で送信挙動は変えていない。詳細は `docs/283`。
- **前回リリース（2026-09-18）**: 管理画面のメール通知設定をカテゴリ別に色分け。14 枚すべて同じ紫で種類が判別できなかった状態を解消し、「その他の自動通知」6 枚を 研修 / 認証・セキュリティ / 会員手続き の 3 グループへ分割した。表示のみの変更で送信挙動は変えていない。詳細は `docs/282`。
- **前回リリース（2026-09-16）**: 通知設定をメール通知と Google Chat に分離。メール全体制御を停止／通常送信／テスト集約へ一本化し、Chat の受付・確定を申請IDごとの同一スレッドへ集約した。既存メール設定を止めない互換読み替えを追加。詳細は `docs/281`。
- **直近の本人確認・承認機械チェック（2026-09-15）**: `test:public-identity` 15/15 PASS。賛助会員の公開本人確認項目にCM番号が含まれないことに加え、承認保存処理がログインIDをCM番号として検証しないことを固定した。
- **検証**: v376.95.1 の `prerelease` PASS、3 split 生成物検査、管理画面7 viewportレスポンシブ、deployment API の4本同期を完了。失敗していた既存の賛助会員変更申請は未処理のまま保持し、修正後に通常の承認操作で再試行できる。
- **検証は 1 ページで見られる**: [`docs/portal/test-report.html`](docs/portal/test-report.html)。
  再生成は `npm run report:tests`。

### 0.1 次担当者への申し送り（v376.98 / v376.97 / v376.96 / v376.95.1）

-2. **v376.98 は本番確認済み**: 本番で実操作し、本文のカーソル位置へ挿入されること（末尾追記ではないこと）を確認した。一括メール送信画面の差し込みには未適用。
-1. **v376.97 は本番確認済み**: 管理レスポンシブ 7 viewport × 8 view PASS、メール通知タブの系統選択を実画面で確認した。
0. **v376.96 は本番確認済み**: 管理レスポンシブ 7 viewport × 8 view PASS、メール通知タブの 6 グループ配色を実画面で確認した。残りは「事業所会員メール設定（統合済み）」の旧見出しに配色未適用であること（`docs/spec/04_UIUX.md` §11 の未確定事項）。
1. **実通知の最終確認は未実施**: コード・本番設定画面・レスポンシブは検証済みだが、今回のリリースでは入会／変更／退会を新規作成してメールや Chat を送るテストはしていない。実行する場合は operator の明示承認、対象データの後片付け、宛先の確認を先に行うこと。
2. **メール抑止の確認候補**: v376.95 初回反映は、既存の通常送信設定を停止として解釈し得る初期値を含んでいた。v376.95.1 で同一作業内に修正済みで、現行画面は通常送信を正しく読み戻すことを確認した。ただし切替までに発生した自動メールがあれば抑止された可能性がある。必要時だけ送信ログを**読み取り専用**で確認し、再送は operator 判断なしに行わない。
3. **既存の変更申請を保全**: 賛助会員の失敗していた変更申請は未処理のまま保持している。通常の承認で再試行できるが、テストデータとして削除・却下しない。
4. **作業ツリーは意図的に dirty**: 過去リリース・退会方式・通知機能・文書整理の変更が未コミットで混在している。次担当者は `git reset`／一括破棄をせず、対象差分だけを確認すること。
...
| 3 | `! node scripts/auth-bootstrap-admin-auto.mjs` | 管理ポータルの E2E | responsive:admin が全 viewport `App frame did not appear` |
...
v264 で本人確認が作り直された際、**画面からは切り離されたまま、外部から呼べる状態で残った**。

| | 現行（v264〜・v376.88 で拡張）| 旧（残存）|
|---|---|---|
| 本人確認 | 名義 ＋ 番号 1 つ ＋ 返信先メール | **CM番号 8 桁のみ**（名義照合なし）|
| 試行回数制限 | 15 分 5 回 | **なし** |
| 書き込み先 | `T_変更申請`（PENDING）| **`T_会員` を直接更新** |
| 管理者承認 | 必要 | **不要** |

`submitPublicWithdrawalRequest` だけは、発行側が HMAC トークンへ移ったのに検証側が旧 CacheService
...
#### 【operator 確認待ち】実データを使う確認（実装は完了）

1. **本人確認を実データで 1 回通す。** 管理画面または gitignored のローカル設定から必要な値を operator 自身が参照し、画面で確認する。認証情報・会員識別子・連絡先を文書やチャットへ残さない。
2. **職員の追加・情報変更・除籍の実地検証。** `scripts/verify-staff-flow.mjs <add|update|remove> <事業所番号> <返信先メール> [--submit]` を使う。職員追加は認証アカウントを作るため、後片付けまで含めて計画する。
   本人確認のレート制限（15 分 5 回）に当たりやすいため、1 セッションで本人確認は 1 回に収める。
...
- 不要になったら `previewStrictE2ETestMemberCleanup_LOG` で件数を確認し、
  `executeStrictE2ETestMemberCleanup_APPLY` を Apps Script エディタから実行する。
  `test-member-*.invalid` の厳格一致だけを対象に、会員・認証・職員・変更申請を soft delete する。旧デモ／外部申込者は対象外。
...
#### 【C】管理画面からのロック解除 — **完了（v376.78・2026-09-05）**
...
| **E2E でダイアログを一律に握りつぶす** | 保存が失敗しても画面上は成功に見える（v376.86 の一括保存が本番で 1 件も保存できていなかったのを見逃した） | **全ダイアログを記録したうえで accept する**。dismiss にすると承認ボタンの `confirm()` まで打ち消され、押したのに何も起きない状態になる（今度はこれで空振りした） |
| **`processApiRequest` の分岐で存在しない変数を使う** | 実行時 `ReferenceError` で action が丸ごと落ちる。typecheck は `.gs` を見ず、単体テストは分岐を評価しないので通ってしまう | 管理者セッションは **`parsedPayload.__adminSession`**。`npm run test:action-dispatch` が裸の `adminSession` を検出する |
| **`memberType === 'INDIVIDUAL' ? A : B` と書く** | 種別が増えた瞬間、賛助会員が事業所側へ落ちる。v376.88 で 4 箇所見つかった（許可リスト 2・画面の項目・入力収集） | **種別 → 値の表を引く**。`PUBLIC_UPDATE_ALLOWLIST_BY_TYPE_` の形にする |
| **Tailwind のクラス名を組み立てる** | `bg-${tone}-50` は v4 のスキャンに引っかからず**クラスが生成されない**（無地の画面になる） | 用途ごとに完成形の文字列を持つ表にする |
| **`input[type=text]` で入力欄を拾う** | type 属性を書いていない `<input>` は 1 件も取れず、fill が黙って空振りする | `getByPlaceholder` かラベルで狙う |
| **フォームの「最初の空欄」に値を入れる** | 「空欄＝変更なし」の画面は全欄が空で始まるため、電話番号のつもりの値が**事業所名**に入った（承認していれば事業所名が化けていた） | ラベルで項目を狙い、**入力後に読み返して一致しなければ送信前に落とす** |
| **公開ポータルの本人確認を E2E で連投する** | 同一 ID につき 15 分 5 回。6 回目から `waitForSelector` のタイムアウトになり、セッション切れやセレクタ崩れと区別がつかない | ページから `試行回数` の文言を拾って確認する。**1 セッションで本人確認 1 回**に収める設計にする |
| **管理 E2E の認証を ENTER 待ちのスクリプトで取る** | バックグラウンドに回ると入力を渡せず固まる | `scripts/auth-bootstrap-admin-auto.mjs`（ログイン後の状態を 5 秒ごとに保存）を使う。**文言で「管理 UI が出た」と検知する方式は空振りする**ので採らない |
...
- **テンプレートが重複を作っていた**。入力バリデーションを 3 箇所、監査ログを 4 箇所、権限を 4 箇所に
  書かせる構造で、「5 文書を統合しない」という分離の意図と矛盾していた。
- **コードが正本である前提が無かった**。スキーマ表・API 契約・入力規則を手書きさせるが、
  この案件では自動生成・型検査・単一情報源テストで守られており、手写しは腐るコピーにしかならない。
- **必要な節が無かった**: スキーマ移行と戻し方／検証方法／改訂履歴／用語集／既存システムの差分列挙。

**改訂**: `docs/266`（プロンプト v3.0）と `docs/267`（テンプレート v3.0）を新設。
**正本マトリクス方式**（主題ごとに正本を 1 つ決め、他文書は参照のみ）へ切り替えた。
Workflow に Step 0.5（仕様とコードの差分列挙）と Step 9（整合確認）を追加。

**5 文書への反映**（足すべきものだけ。書き写しは足していない）

| 文書 | 版 | 追加 |
|---|---|---|
| SOW | 1.1 | **用語集 TM-01〜13**／非機能に **NF-01〜11 と検証方法の列**（「未検証」も理由つきで明記）|
| RD | 1.2 | 業務ルールに **BR-01〜18**／検証方法の一覧 |
| TRD | 1.3 | **§17 スキーマ・設定の変更手順（MG-01〜03・戻し方つき）**／§18 移行期の並走 |
| UI/UX | 1.1 | **画面 ID SCR-01〜63（遷移元・表示条件つき）**／§5 画面を止める条件／§6 通知の規約 |
| データIF | 1.1 | **§9 本書に置かない節と、その正本**（列定義・API 個別契約・入力規則の値）|
...
- 検証: prerelease 18 スイート PASS。公開ポータル a11y 違反 0 / responsive 7/7 PASS。
  管理ポータル responsive **56 view 全 PASS**（console error 0）。データ出力の画面のパンくず表示も実機で確認（この画面は responsive テストの対象外のため個別に検証）。
...
- **GCP 移行の前提機能 B**（`docs/261` T-07）。一括編集で直せるのは 6 項目だけだったため、
  **連絡先（勤務先電話・FAX・携帯）／勤務先（事業所名・郵便番号・都道府県・市区町村・住所・建物名）／
  自宅（郵便番号・都道府県・市区町村・住所・建物名）** の 14 項目を追加した（計 20 項目）。
- 画面は列数が増えすぎないよう**折りたたみ式**（「連絡先・住所も編集する」で展開）。
- **事業所職員は対象外**。`T_事業所職員` は電話・住所の**列そのものを持たない**（連絡先は所属事業所側が正本）ため、
  許可リストにも入れていない。列を増やす場合はスキーマ変更が先。
- **意図的に対象外**: 氏名・カナ・介護支援専門員番号・会員種別・職員数上限（本人特定やログイン ID に直結し、
  一括で書き換えると事故が大きい）。unit test で書き換え不可を固定した。
- 本番 fixed deployment は **public @378×2 / member @137 / admin @234** に同期済み。
- live 検証: 本番 API が拡張 7 項目を返し**職員行には含まれない**ことを実測（345 件）。
  admin responsive 56 view・メール設定 E2E 5/5・公開 a11y 0・responsive 21 view。
- 新ゲート `test:batch-edit`（7 件）を prerelease に追加。

## 2026-09-03 v376.68 / .68.1 / .68.2 リリース（汎用データエクスポート）

- **GCP 移行の前提機能を GAS 側で先行実装**（`docs/261` T-07・operator 判断）。移行後はスプレッドシートを
  直接開けなくなるため、任意のテーブルを CSV で取り出す手段を先に用意した。
- 管理ポータル → システム → **データ出力（CSV）**。**既定は MASTER のみ**で、事務局へ渡す場合は
  MASTER が 権限管理 から `data-export` メニューを明示付与する（会員の個人情報を丸ごと持ち出せる操作のため）。
- 安全設計: **認証テーブルとそのアーカイブは MASTER でも出力不可**／ログ系と設定は MASTER 限定／
  未知テーブル名は拒否／**持ち出しを監査ログに記録**（中身は残さない）／CSV インジェクション対策／1 回 20,000 行の上限。
- **途中で 2 件の不具合を検出・是正**: ①`T_認証アカウント_archive` が出力対象に混入（完全一致判定だったため）
  →`_archive` を外して判定する共通関数へ ②一覧表示に 20 秒→**5.5 秒**（56 シート分の行数取得をやめた）。
- **ビルドの罠**: 二重引用符を含む正規表現を書くと関数抽出パーサが壊れ、以降の関数が生成物から消える
  （operator ツール 4 件が消失して発覚）。正規表現を使わない実装に変更し、ソースに警告コメントを残した。
- 本番 fixed deployment は **public @377×2 / member @136 / admin @233** に同期済み。
- live 検証: dryRun 10 チェック全 PASS／管理画面の実動作 PASS／**実 CSV の中身まで確認**／
  admin responsive 56 view／メール設定 E2E 5/5／公開 a11y 0・responsive 21 view。
...
- **根本原因**: 承認メールの送信経路が会員種別で 2 本に分かれており、事業所側（`renderBizEmailTemplate_`）に
  両変数を渡していなかった。同関数は渡された key だけを置換するため、未知のタグは原文のまま残る。
  値そのものは直前で `M_会員種別` から取得済みで、個人向け `credEmailOpts` にだけ載せていた取りこぼし。
- **修正**: ①事業所メールへ `会員種別` / `年会費` を追加 ②年会費の整形を `formatAnnualFeeForMail_` に共通化
  ③**全メール共通の出口 `deliverMail_` で未解決タグを送信直前に除去**（ログに category とタグ名を残す）
  ④管理画面のタグ案内を送信側と一致させた。
...
- **規程・重要事項マスタ `T_規程` を新設**（案C Phase 1）。入会申込画面の「事務局からのお願い」と定款リンクは
  従来フロントにハードコードされていて事務局が改定できなかった。**本文の正本を DB の 1 箇所へ移し**、
  管理画面 設定 → **規程・重要事項** から追加・編集・削除・公開/非公開・表示順を操作できるようにした。
- 初期データは**現行のハードコード文面 5 件をそのまま seed**（内容は従来と同一）。空のときだけ seed し、改定を上書きしない。
- 版数は**タイトル・本文・リンクが変わった保存でだけ +1**（Phase 2 の同意記録が「どの版に同意したか」を指すため）。
- **同意記録（`T_規程同意`）は Phase 2 で未着手**。着手時は cascade アーカイブ登録・ER リレーション・GCP 移行方針の
  3 点を必ず処理する（`docs/258` §2）。
- **落とし穴 2 件**（いずれも新設ゲートで再発防止済み）:
  1. **`DB_SCHEMA_VERSION` の bump 漏れ**（@370）。テーブルが作られず、公開側はフォールバック文面を出すため画面上は正常に見えていた（dryRun で検知）。v376.65.1 @371 で解消。
  2. **admin split の action 許可リスト（`ADMIN_ALLOWED_ACTIONS_LIST`）への登録漏れ**（@371）。`ACTION_TO_MENU` と `ADMIN_ACTION_PERMISSIONS` には入れたが許可リストに無く、
     実行時に `未実装アクションです` が返って管理画面が「読込中...」のまま固まった。v376.65.2 @372 で解消し、
     **`test:menu-registry` に「ACTION_TO_MENU の admin action は許可リストに登録されている」ゲートを追加**した。
...
- **管理画面の日本語表記統一**: 画面タイトル上の装飾英語 `System Settings` を削除、`allowlist`→「許可リスト」、`ON/OFF`→「有効／無効」、配信モード `LIVE/REDIRECT/SUPPRESS` を
  「通常送信／テスト集約／送信抑止」＋英字併記（日本語を主・英字を従）へ。`Step 1/2/3`→「手順1/2/3」、`Visual Designer`→「レイアウト設計」、`FROM/TO`→「開始／終了」、
  `AND`/`NOT`/`opt-out`/`SOW` 等の社内英語も平文化。日本語要素に効いていなかった `uppercase` 28 箇所も除去。対象 17 ファイル。
- **語句ルールを明文化**: `AGENTS.md` §4.4 の先頭に「画面表示は日本語を既定とする（装飾英語禁止・英字は内部値/識別子と定着語のみ・その場合も日本語を主）」を新設。
- 公開ポータルの研修カード補助ラベル既定値のみ `TRAINING`→`研修申込`（**本番 DB に保存済みの値は変わらないため、必要なら 設定→公開ポータル で上書き保存**）。
- 本番 fixed deployment は **public @368×2 / member @127 / admin @224** に同期済み。
- live E2E: 公開 a11y 違反 0・公開 responsive 7VP・member responsive 7VP×3画面・admin responsive 7VP×8 コンソール=56 view・メール設定 E2E 5/5 いずれも PASS（console error 0）。
...
- **【最重要・2026-09-03 operator 決定】GCP 移行は一旦中断。新規仕様は本リポジトリ（GAS）側で実装する。**
  ただし**再開前提**であり、これ以降に実装する仕様には **GCP 移植可能性ゲート**が課される
  （`AGENTS.md` §4.8・GCP 作業場 `AGENTS.md` §1-B に同期コピー）。
...
- **⚠️ 現在の作業拠点は GCP 作業場（`C:\VSCode\CloudePL\hirakatacitykyougikaiGCP`）。ただし本番主系は依然 GAS＝この本番リポジトリは凍結しない**（2026-08-02 判断）。理由: 移行は Phase 4 の入口（member 認証パイロット＋低リスク write 準備）までで、**ユーザー系 write は 1 件も GCP へ移っていない**（`docs/250` §6.1 棚卸し: P4-write **66 method** 未着手・4c 第一号 write=共有メモは operator 承認 #5 待ち）。Phase 5（DB 移行）・Phase 6（切替/GAS 縮退）は未着手で、GAS 3 split が全機能の唯一の稼働系。**GAS 側の「保守モード」は 2026-09-02 operator 決定で解除した**（理由: 運用を継続する必要があり、GCP へ移し終えるまでは本リポジトリ側での新規実装も必要になったため）。以後は**通常開発モード**で新規機能追加・改修を受ける。ただし GAS へ write 系を足すと `docs/250` §6.1 の未移行 write（66 method）が増えるため、**大型の新規 write 機能は着手前に「GAS/GCP どちらで作るか・二重実装の工数」を operator に提示して方針確認する**（小〜中規模・障害修正・運用要求は確認不要）。**凍結（変更禁止）ではない**ため、リリース時の完了条件（prerelease 全ゲート・3 split 同期・live E2E）は従来どおり必須。**ルール正本は `AGENTS.md` §4.7**。凍結判断は Phase 6 の旧 URL 転送化（`docs/250` §5 Phase 6）と同時に行う。
...
- **まず読む**: 本書 §0→§1→§2→`docs/250`（GCP並走移行計画・正本）→ **GCP 作業場 `docs/HANDOFF_2026-07-25_member-auth.md`（現在の中断点＝再開の一手）**→ GCP 作業場 README/AGENTS/`docs/PHASE4_DESIGN.md`→`docs/240` §11（password-hash Phase B詳細）。削除設計は `docs/249`、第三者評価は `docs/248`。落とし穴は MEMORY。
...
> **🚩 次の一手（2026-09-02 時点）**
> 1. **保守モードは 2026-09-02 に解除（operator 決定）。本リポジトリは通常開発モード**＝新規機能追加・改修を通常のリリースフローで受ける（ルール正本 AGENTS §4.7）。大型の新規 write 機能だけ、着手前に GAS/GCP のどちらで作るかを operator に確認する。
> 1-b. **v376.63 デプロイ済（2026-09-02）**: 管理画面の日本語表記統一。live E2E は公開／member／admin の 3 split すべて PASS（下記 §2-1）。残作業は公開ポータルの研修カード補助ラベル（DB 保存値が `TRAINING` のままなら 設定→公開ポータル で上書き）だけ。
...
>
> **作業を始める前に**: 管理画面の Playwright E2E を回すには admin の Google セッションが要る。`node scripts/auth-bootstrap-admin-auto.mjs` を実行して operator がログインすると `.test-out/auth-admin.json` が更新される（**セッションは短命で、同日中に失効することがある**）。
...
> **仕様書の巻き直し（2026-09-03 着手）に関する差分・不足・タスクは
> [`docs/261_SPEC_CODE_DIFF_2026-09-03.md`](docs/261_SPEC_CODE_DIFF_2026-09-03.md) に一本化している**（本表には再掲しない）。
> 現在 差分 D-01〜D-08 / 不足 G-01〜G-13 を登録済み。判定は棚卸し完了後にまとめて operator へ依頼する。
...
| ✅ | **管理ポータルの live 検証（完了）** | responsive 56 view 全 PASS・console error 0。データ出力の画面のパンくずも実機で確認。fatal でも exit 0 を返していたゲートは修正済み | `docs/264_RELEASE_STATE_v376.70_2026-09-04.md` |
