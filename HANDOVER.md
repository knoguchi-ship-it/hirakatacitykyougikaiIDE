...
- **本番**: public **@407×2** / member **@165** / admin **@262**（**v376.97**）。4 本を同一リリースへ切替済み。
  ロールバック先は public @406×2 / member @164 / admin @261（v376.96）。Apps Script API の一覧で 4 本が現行版を指すことを確認済み。
- **直近リリース（2026-09-22）**: 「システム設定 ＞ メール通知」の「メール通知の内容」を系統選択式にした。6 系統のボタンを先頭に置き、選んだ系統のカードだけを表示する（一度に出るカードは最大 14 枚 → 3 枚）。「メール配信」は独立したまま。表示のみの変更で送信挙動は変えていない。詳細は `docs/283`。
  **本番の実画面確認は未実施**（ブラウザ認証切れ。§1 の 3 番を実行してから確認する）。
...
### 0.1 次担当者への申し送り（v376.97 / v376.96 / v376.95.1）

-1. **v376.97 の本番実画面確認が未了**: `! node scripts/auth-bootstrap-admin-auto.mjs` で認証を取り直し、管理画面 ＞ システム設定 ＞ メール通知 で ①「メール配信」が独立して残っていること ②系統ボタン 6 個と選んだ系統のカードだけが出ること ③入会・職員追加を選んだときだけ共通スイッチが出ること を確認する。あわせて `npm run test:responsive:admin`。表示のみの変更なのでロールバック緊急度は低い。
