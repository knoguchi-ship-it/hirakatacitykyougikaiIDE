...
> **後任の着手手順**: ①`docs/250` §12 を読む（動機＝ロード時間短縮・確定事項・検証済み事実・コスト早見）②~~着手前に現行の実データ量と実ロード時間を実測~~ **→ 完了（2026-07-11・§12.6 に記録）**: 会員222・職員157・研修4（無料枠に余裕）／A=public 3s・admin 8〜12s／B=1呼び出し 1.8〜5s（26 bytes でも ~2s＝固定オーバーヘッド支配）・fetchAllData 4.9〜11.1s → **A・B 両方が支配的＝確定構成の妥当性を数値で裏付け**③~~Phase 1（非破壊）＝`createApiClient`/`GcpApiClient` の器づくり~~ **→ v376.57 で完了（2026-07-11・GAS E2E 通過済）**④公開前に §11-1 DoW/EDoS 対策を消化。**次は Phase 2（GCP 作業場）**＝Firestore データモデル設計＋read-only 互換 API（§12.7-2。着手時決定事項: 移行期に「スプレッドシート読取の互換層」か「早期 Firestore 同期」か）。
