...
- **🚀 GCP 移行: 全体計画の正本は `docs/250`。Phase 0（GCP 側）全完了・次は Phase B（本番 GAS 接続リリース）** — 作業場は **`C:\VSCode\CloudePL\hirakatacitykyougikaiGCP`**（独立Git・GitHub private `knoguchi-ship-it/hirakatacitykyougikaiGCP`）。**本番完全分離**（切り分け規約=同作業場 AGENTS.md §1 が正本・GCP リソースは追加のみ）。
  ...
  - **Phase B 実装着手（2026-07-07・未 push/未リリース）**: ①3 split manifest に `openid` 追加＋public/member に `userinfo.email` 追加（設計決定: `ALLOWED_INVOKERS` 維持・Secret 名は GAS 側を GCP 実体に合わせる）②secret 名是正（既定 `PASSWORD_HASH_PEPPER_V1`・Script Property `PASSWORD_HASH_PEPPER_SECRET_NAME` で上書き可）③`dryRunGcpPhaseB_LOG` 新設（admin build のみ・identity token の aud/iss/email 有無＋Secret Manager 取得可否＋Cloud Run `/health` 診断。token/pepper 値は出力しない）。build×3 生成物残存確認＋prerelease 全 PASS 済。**次: operator 承認後に 3 split `clasp push`（scope 変更＝operator 再同意発生・匿名利用者影響なし）→ admin editor で `dryRunGcpPhaseB_LOG` ▶ → 実測 aud を Cloud Run custom audiences へ登録**（member/public の aud は admin 実測後に GCP コンソール credential 一覧で照合）。
