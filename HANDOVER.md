...
## ▶ 再開プラン（2026-09-10 時点・次の担当者はここから）

### 0. 30 秒で現状

- **本番**: public **@400×2** / member **@158** / admin **@255**（**v376.91**）。4 本を同一リリースへ切替済み。
  ロールバック先は public @399×2 / member @157 / admin @254（v376.90）。Apps Script API の一覧照合は、デプロイ権限のある Google アカウントで再認証後に 1 回行う（現ログインアカウントは対象プロジェクトの権限不足）。
- **直近リリース（2026-09-10）**: v376.90 で公開本人確認の設定が build pruner により欠落する障害を修正し、内部例外を画面へ出さない日本語メッセージ変換と生成物検査を追加した。v376.91 で公開ポータルの補助ラベルのコントラストを WCAG AA に適合させた。詳細は `docs/276` / `docs/277`。
- **直近の本人確認機械チェック（2026-09-10）**: `test:public-identity` 14/14 PASS。公開生成物から抽出した本人確認は一致時に成功・トークン発行、不一致時に安全に失敗することを確認した。本番公開画面でも架空の不一致入力で日本語の案内・console error 0 を確認し、申請は送信していない。実会員情報による成功確認は operator が 1 回実施する。
- **検証**: v376.91 の `prerelease` PASS、公開 a11y は 3 view で違反 0、公開 responsive は 320〜1920px の 7 viewport PASS。退会申請の実送信は行っていない。
