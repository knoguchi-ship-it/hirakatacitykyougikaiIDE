...
## ▶ 再開プラン（2026-09-16 時点・次の担当者はここから）

### 0. 30 秒で現状

- **本番**: public **@405×2** / member **@163** / admin **@260**（**v376.95.1**）。4 本を同一リリースへ切替済み。
  ロールバック先は public @403×2 / member @161 / admin @258（v376.94）。Apps Script API の一覧で 4 本が現行版を指すことを確認済み。
- **直近リリース（2026-09-16）**: 通知設定をメール通知と Google Chat に分離。メール全体制御を停止／通常送信／テスト集約へ一本化し、Chat の受付・確定を申請IDごとの同一スレッドへ集約した。既存メール設定を止めない互換読み替えを追加。詳細は `docs/281`。
