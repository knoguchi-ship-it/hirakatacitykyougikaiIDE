...
1. **賛助会員の本人確認を実データで通す。** テスト賛助会員は `テスト 会員20260904`（会員番号 67735027）。
   照合には**登録されている携帯電話番号**が要る（管理 → 会員一覧 → 詳細で確認）。
   電話番号は空、CM番号は賛助会員には出ないので、**携帯番号でしか通せない**。
2. **職員の追加・情報変更・除籍の実地検証。** `scripts/verify-staff-flow.mjs <add|update|remove> <事業所番号> <返信先メール> [--submit]`。
   テスト事業所は `テスト枚方`（事業所番号 5770806521・枠は 9 名空き）。
   **職員追加は認証アカウントを作る**ので、後片付けまで含めて計画すること。
   本人確認のレート制限（15 分 5 回）に当たりやすいので、**1 セッションで本人確認 1 回**に収める。
