...
- **本番**: public **@408×2** / member **@166** / admin **@263**（**v376.98**）。4 本を同一リリースへ切替済み。
  ロールバック先は public @407×2 / member @165 / admin @262（v376.97）。Apps Script API の一覧で 4 本が現行版を指すことを確認済み。
- **直近リリース（2026-09-22）**: メール通知の差し込みを一覧表示からクリック挿入のボタンへ変更。あわせて、表示タグと実際に使えるタグの不一致（事業所・メンバーのタグ未表示／職員追加・ワークフローで和集合を表示）を解消した。詳細は `docs/284`。
