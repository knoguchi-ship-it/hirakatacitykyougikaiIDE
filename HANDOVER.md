...
## ▶ 再開プラン（2026-09-18 時点・次の担当者はここから）

### 0. 30 秒で現状

- **本番**: public **@406×2** / member **@164** / admin **@261**（**v376.96**）。4 本を同一リリースへ切替済み。
  ロールバック先は public @405×2 / member @163 / admin @260（v376.95.1）。Apps Script API の一覧で 4 本が現行版を指すことを確認済み。
- **直近リリース（2026-09-18）**: 管理画面のメール通知設定をカテゴリ別に色分け。14 枚すべて同じ紫で種類が判別できなかった状態を解消し、「その他の自動通知」6 枚を 研修 / 認証・セキュリティ / 会員手続き の 3 グループへ分割した。表示のみの変更で送信挙動は変えていない。詳細は `docs/282`。
  **本番の管理画面レスポンシブ検査だけ未実施**（ブラウザ認証切れ。§1 の 3 番を実行してから `npm run test:responsive:admin`）。
...
### 0.1 次担当者への申し送り（v376.96 / v376.95.1）

0. **v376.96 の本番表示確認が未了**: `! node scripts/auth-bootstrap-admin-auto.mjs` で認証を取り直し、`npm run test:responsive:admin` を実行して `.test-out/result-admin.json` の `fatal` / `consoleErrors` / `views[].error` で判定する。配色は表示のみの変更なのでロールバック緊急度は低い。
