...
## 現在の確定運用状態
- ブランチ: `main`
- 本番反映（固定 Deployment）: `@146`（2026-03-28 両 Deployment 同期済み）。Version 151 作成済み、@151 同期はユーザー作業待ち
- v118〜v146 が deployment 反映済み。v147〜v151 は clasp push 済み・Git 未コミット
- v147: 対象年度フィルタ+一括編集下部ボタン+退会済み事業所バリデーション緩和、v148: 退会済み全会員種別の必須項目緩和、v149: ログインUX改善、v150: キャッシュチャンク+複合API+スケルトン画面、v151: ダッシュボードキャッシュ二重パースバグ修正
