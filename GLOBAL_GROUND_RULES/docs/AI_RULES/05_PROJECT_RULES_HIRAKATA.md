...
## 現在の確定運用状態
- ブランチ: `main`
- 本番反映（固定 Deployment）: `@145`（2026-03-28 両 Deployment 同期済み）
- 5 段階権限モデル（v118〜v120）、年会費コンソール改善（v122〜v124）、管理コンソール会員管理改善（v125）、事業所会員詳細編集改善（v126）、職員詳細画面改善（v127）、データ移行（v128）、年度対応・事業所整理（v131）、入会日補正（v132）、メール配信希望（v133）、careManagerNumber修正・連絡設定非表示（v135）、権限階層修正・インライン職員編集・サイレントリロード（v136〜v140）、`selectedMemberForDetail` の ID 派生化による `S-04` 修正（v141）、管理者フィールド編集オーバーライド（v143）、ダッシュボード在籍会員のみ（v144）、事業所番号投入・年会費ナビ・サイドバー修正・バリデーション緩和（v145）が反映済み
